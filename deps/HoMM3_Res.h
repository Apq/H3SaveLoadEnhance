...
NOALIGN struct _BinTreeItem_ // size = 28
{
 ...
 // 愉嚯栩?耨牦 磬 钺牝 ? 羼腓 铐?镱耠邃?, 箜梓蝾蜴栩?邈?
...
{
 _dword_ PlayingIndex; // + 28 = 0 镳?玎沭箸赍; 桧溴犟 耩邃?镳铊沭帼躐 玮箨钼
 _ptr_ data; // + 32 - 箨噻囹咫?磬 玎沭箧屙眍?耦溴疰桁铄 wav-羿殡?
 _dword_ data_size; // + 36 疣珈屦 溧眄 (疣珈屦 wav-羿殡?
 _dword_ field_28; // + 40 = 0 镳?玎沭箸赍 // 镱耠?桧桷栲腓玎鲨?玮箨钼 祛眈蝠钼 = 3 
 _dword_ field_2C; // + 44 = 127 镳?玎沭箸赍 // 镱耠?桧桷栲腓玎鲨?玮箨钼 祛眈蝠钼 = 64
 _bool32_ no_loop; // + 48 = 1 镳?玎沭箸赍 // 镱耠?桧桷栲腓玎鲨?玮箨钼 祛眈蝠钼 = 0 ?玮箨?move 祛眈蝠??= 1 ?铖蜞朦睇?玮箨钼 祛眈蝠?
};
...
{
  _Wav_* Wav; // +0h; 氢箨
  _dword_ PlayingInd; // +4h; 软溴犟 耩邃?镳铊沭帼躐 玮箨钼
};
...
NOALIGN struct _Def_ : public _BinTreeItem_
{
 ...
 // 悟痂耦怅?def`??镱塍镳铉疣黜铖螯? 橡?怅膻麇龛?镱塍镳铉疣黜铖蜩 桴钺疣驽龛?镱腠铖螯?耱囗钼栩? 镱塍镳铉疣黜.
 // 锐镱朦珞弪? 潆 铗痂耦怅?囗桁圉栝 ?犷?
 ...
 // 悟痂耦怅?def`??镱朦珙忄蝈朦耜桁 耧弼鲡弪铎, 玎戾?桁? 磬 箨噻囗睇?
 // 锐镱朦珞弪? 潆 耱尻钼 ?犷?(疣礻?忸牮筱 耱?泐 耱尻?.
 ...
 // 悟痂耦怅?def`?(蜞, 黩?桉镱朦珞弪? 潆 铗痂耦怅?耥囵漕??犷?(忸珈铈磬 徨?桉镱朦珙忄龛 耧弼鲡弪钼)).
...
NOALIGN struct _Fnt_:  public _BinTreeItem_
{
 ...
 _byte_ field_24[5]; // +28
 
 // 蔓耦蜞 耱痤觇.
 _byte_ string_height; // +33
 
 _byte_ field_30[3]; // +34
 
 _byte_ unknown[23]; // +37
 
 _CharSize_ char_sizes[256]; // +60
